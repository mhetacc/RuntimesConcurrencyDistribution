...
It is not our intention to plagiarize Ongaro and Ousterhout's excellent work \textit{"In Search of an Understandable Consensus Algorithm"} \cite{raft} by presenting the Raft algorithm's specifications. Instead, we \textcolor{changes}{are going to} discuss how we molded it to our own use case.

The algorithm \textcolor{changes}{divides its nodes into three roles}, namely \textit{leader}, \textit{follower} and \textit{candidate}, and revolves around three core functionalities: leader election, log replication and cluster membership change. Log compaction is also mentioned, while a byzantine fault tolerant variant is never explored by the original authors. \textcolor{changes}{To grant consistency, Raft's design choice is to centralize all decisions on one node, the above mentioned leader, that synchronizes all cluster's nodes}.
...
All nodes, except for the leader, \textcolor{changes}{perform} as followers. They are not allowed to replicate their own log, and they have to forward any request to the leader.
...
When a follower's election timeout times out, it becomes a candidate, increments its own term and starts an election. Votes for itself and then waits for \textcolor{changes}{one} of two outcomes: wins, thus becoming a new leader, or loses (either another leader gets elected or the old one manifests itself) thus reverting back to being a follower.
...
The leader (each \textcolor{changes}{node} as a matter of fact) periodically checks whether there are new commands to propagate (always stored in queue \textit{pygame\_commands}, more details in section \ref{sec:raftian}), by overriding \textit{SimpleXMLRPCServer}'s method \textit{service\_actions} (listing \ref{code:addCommands}).

Then, \textcolor{changes}{it} translates them into entries by giving each of them the current term and a log index that starts from $lastLogEntry(index) + 1$ and increases by one for each entry. To clarify: if $lastLogEntry(index) = 7$ and we have three new commands, their \textcolor{changes}{indices} will respectively be eight (8), nine (9) and ten (10). The translation can be seen at listing \ref{code:translateCommands}.
...
What happens if the \textit{append entries} gets rejected? The leader adds to \textit{new\_entries} its own last log entry: $new\_entries = lastLogEntry + new\_entries$ (figure \ref{fig:newEntries}). Then, \textcolor{changes}{it} repeats the propagation procedure, for each reject a new \textit{last log entry} gets added, progressively traversing the log backwards. If, at a certain point, $new\_entries == allLog + new\_entries$ (i.e., all leader's log gets propagated) the flag \textit{all\_log} is set to \textit{True}. 
...
\begin{figure}[h]
  ...
  \caption{New entries with the last log's entry \textcolor{changes}{pushed to the top} of the list}
...
This functionality, \textcolor{changes}{as well as the following ones}, have not been implemented due to time constraints. We decided to include them anyway since they were still the product of careful consideration and could prove useful in future implementations.

Log compaction, also called \textit{snapshot}, is a way to \textcolor{changes}{clear} servers' logs and save them in persistent memory, necessary in long-running or message-heavy applications. Every node autonomously decides when to do it.

\textcolor{changes}{The idea is as follows}: inside their function \textit{service\_actions}, servers check whether their log is larger than a certain size (\textcolor{changes}{to be determined}) and then call a \textit{snapshot} method accordingly, which saves all entries in a JSON file progressively deleting them from the log, from the first one up to (but excluding) the \textit{last applied}.

An alternative interpretation, closer to Ongaro and Ousterhout's idea, is saving the state of the cluster. When applied to our context, we can imagine a JSON file that describes \textcolor{changes}{all servers'} state, by saving for each of them \textit{id}, \textit{url}, \textit{port}, and \textit{hp} values. \textit{Index} and \textit{term} of the last snapshotted entry should also be saved, as well as the current configuration (which will be mentioned in section \ref{sec:raftMissing}). An example of the JSON structure can be seen at listing \ref{code:snapJson}. 
